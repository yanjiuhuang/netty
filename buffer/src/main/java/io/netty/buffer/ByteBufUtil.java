...
public final class ByteBufUtil {
    ...
    /**
     * Write an <a href="http://en.wikipedia.org/wiki/UTF-8">UTF-8</a> {@link CharSequence} into a {@link ByteBuf}.
     */
    public static void writeUtf8CharSequence(ByteBuf buf, CharSequence seq) {
        if (buf == null) {
        ...
        // UTF-8 uses max. 3 bytes per char, so calculate the worse-case.
        ...
        if (buf instanceof AbstractByteBuf) {
            ...
            buf.writeBytes(seq.toString().getBytes(CharsetUtil.UTF_8));
        }
    }

    /**
     * Write an <a href="http://en.wikipedia.org/wiki/ASCII">ASCII</a> {@link CharSequence} into a {@link ByteBuf}.
     */
    public static void writeAsciiCharSequence(ByteBuf buf, CharSequence seq) {
        if (buf == null) {
            throw new NullPointerException("buf");
        }
        if (seq == null) {
